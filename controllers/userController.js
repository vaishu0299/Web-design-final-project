...
signup:function(req,res){
      ...
      var regexpassword = /^(?=.*\d)(?=.*[!@#$%^&*])(?=.*[a-z])(?=.*[A-Z]).{8,}$/
           ...
           console.log(password);    
            if(!req.body.username.trim().match(regexusername)){
               res.status(400);
               return res.json({"message":"Email Input does not match"});
           }
           else if(!req.body.password.trim().match(regexpassword)){
                  res.status(400);
                  res.json({"message": "Please enter minimum eight characters, at least one lowercase and uppercase letter"});
             } 
             else{User.find({username:req.body.username}).exec(function(err,user)
             {
               if(err)
               {
                 ...
                 if(err){
                   ...
                   if(err) {
                     //if errors occours during the creation of the user.
                       ...
                       res.status(500);
                       res.json({"status":false,"message": 'Failed to create users'});
                   } else {
                       res.status(200);
         ...
         if(doMatch)
           {
             ...
             res.json({"message":'signin is successful',"accessToken":token,"role":db_user.role})
             console.log(db_user);
             // res.send(db_user);
           }
           else{
 ...
 delete:function(req,res){
     ...
     User.findOneAndDelete({_id:req.user.user_id},function(err,docs)
     {
       if(err)
       {
         console.log(err)
 ...
 confirmPayment:function(req,res){
    const stripe=require('stripe')("sk_test_51KrvpxEzgKwEXW8k2jArJKoroGe43qh0FG1Q8KPZasOAznuF8NKoyrM2G1HusYOTYLmxqfsF8QAOtfi4xd3xXjnD00LNVFoFsY");
     stripe.charges.create({
         ...
         {
             console.log(err);
            return res.json({success:false,status:"payment failed"})
         }
         res.json({success:true,status:"payment success"})
     })
 },
